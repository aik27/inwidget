...
class Instagram
{
    ...
    public function getMedias($username, $count = 20, $maxId = '')
    {
    	...
    	$response = Request::get(Endpoints::BASE_URL.'/graphql/query/?query_id=17880160963012870&id='.$account->getId().'&first='.$count,$headers);
        $arr = json_decode($response->raw_body, true);
        $arr = $arr['data']['user']['edge_owner_to_timeline_media']['edges'];
        if (!is_array($arr)) {
        	throw new InstagramException('Response code is ' . $response->code . '. Body: ' . Instagram::getErrorBody($response->body) . ' Something went wrong. Please report issue.');
        }
        if (empty($arr)) {
        ...
        $medias = [];
        foreach ($arr as $mediaArray) {
        	$mediaArray = $mediaArray['node'];
        	$mediaArray['link'] = Endpoints::getMediaPageLink($mediaArray['shortcode']);
    ...
    public function getMediasByTag($tag, $count = 12, $maxId = '', $minTimestamp = null)
    {
    	...
    	$response = Request::get(Endpoints::BASE_URL.'/graphql/query/?query_id=17882293912014529&tag_name='.urlencode($tag).'&first='.$count,$headers);
    	$arr = json_decode($response->raw_body, true);
    	$arr = $arr['data']['hashtag']['edge_hashtag_to_media']['edges'];
    	if (!is_array($arr)) {
    		throw new InstagramException('Response code is ' . $response->code . '. Body: ' . Instagram::getErrorBody($response->body) . ' Something went wrong. Please report issue.');
    	}
    	if (empty($arr)) {
